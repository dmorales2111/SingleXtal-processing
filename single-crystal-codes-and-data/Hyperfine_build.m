...
Lipc1 = 1e6*mu0*muB^2*S*(S+1)/(3*k*(T - omega))*(ge*LiD*1e30/4/pi); %Pigliapochi term (e) 
Lipc2 = 1e6*mu0*muB^2*S*(S+1)/(3*k*(T - omega))*(giso*LiD*1e30/4/pi); %Pigliapochi term (g)
...
Liso = trace(d)/3; Piso = trace(d1)/3;

LiA = Liso*1e-6*3*k*(T - omega)*gam(1)/(muB*S*(S+1))/(ge+giso)*2*pi;
PA = Piso*1e-6*3*k*(T - omega)*gam(2)/(muB*S*(S+1))/(ge+giso)*2*pi;

save calculated_params.mat LiDeL PDeL Lipc1 Lipc2 Ppc1 Ppc2 ...
    Lidel1 Lieta1 Lidel2 Lieta2 Pdel1 Peta1 ...
    Pdel2 Peta2 Lihyp Phyp Lvec Pvec Liso Piso LiA PA
